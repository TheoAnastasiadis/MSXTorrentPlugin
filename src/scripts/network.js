...
export class Network {
    ...
    MOST_POPULAR_GETAWAYS = [
        ...
        "10.8.0.99",
        "192.168.0.3",
    ] //100 most popular login pages (source: https://www.192-168-1-1-ip.co/default-router-login-ips/)
    ...
    updatePossibleIps = () => {
        this.possibleLocalIps = []
        if (!this.getaway)
            throw "No getaway detected. Impossible to parse local ips."
        ...
        return new Promise((resolve, reject) => {
            ...
            socket.onclose = (event) => {
                ...
                if (end - start < timeout) {
                    //If it closes before timeout, it means the server responded with an error => server exists
                    console.log(`Possible local ip: ${ip}`)
                    resolve(ip)
                } else {
                    //If it closes after timeout, it means it kept trying to connect => no server exists
                    reject(ip)
                }
            }
        })
     ...
     */
    detectGetaway = async () => {
        this.getaway = await Promise.any(
            throttledMap(
                this.MOST_POPULAR_GETAWAYS,
                this.checkAddress,
                this.concurrency,
                this.retryTimeout,
                true
